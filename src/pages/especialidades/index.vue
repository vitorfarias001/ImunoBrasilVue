  ...
  <v-container>
    <v-row no-gutters>
      <div class="header-page">
        <v-col>
          <header>
            <div class="text-h4 text-primary">Especialidades</div>
          </header>
        </v-col>
        <v-col cols="2">
          <v-btn
...
import {
  IEspecialidadeDto,
  IEspecialidadeFiltroRequest,
} from "../../api/types/especialidade";

export default defineComponent({
  components: { SimpleItemTable },
  setup() {
    const LoadingEspecialidades = ref(false);
    const especialidades = ref([] as IEspecialidadeDto[]);
    const especialidadePages = ref({} as IPagedResponseBase);
  ...
  methods: {
    async buscarEspecialidades() {
      ...
      var especialidadeResponse =
        await ApiServices.EspecialidadeApi.BuscaEspecialidade({
          paginacao: { page: 1, pageSize: 50 },
          filtros: {} as IEspecialidadeFiltroRequest,
        } as ISearchDataParams<IEspecialidadeFiltroRequest>);

      this.especialidades = especialidadeResponse.data.results;
      console.log(this.especialidades);
      this.especialidadePages = {
        currentPage: especialidadeResponse.data.currentPage,
        pageCount: especialidadeResponse.data.pageCount,
        pageSize: especialidadeResponse.data.pageSize,
        total: especialidadeResponse.data.total,
      };
